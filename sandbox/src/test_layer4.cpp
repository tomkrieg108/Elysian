...
namespace ely {
	...
	void TestLayer4::OnUpdate(double time_step)
	{
		...
		m_scene->BeginScene("Main Camera"s, glm::vec4{ 0.13f,0.13f,0.13f,1.0f }); //this is going to reset the same view and proj mat in all the shaders!
	...
	bool TestLayer4::OnKeyPressed(ely::EventKeyPressed& e)
	{
		...
		else if (e.key == GLFW_KEY_T)
		{
			...
			Entity controlled_camera_entity = m_scene->GetControlledCameraEntity();
