...
namespace ely
{
		PerspectiveCamera2::PerspectiveCamera2(float aspect_ratio) : m_aspect_ratio{ aspect_ratio }
		{
			SetPosition(glm::vec3(0.0f, 3.0f, 15.0f));
		}

		PerspectiveCamera2::PerspectiveCamera2(float aspect_ratio, glm::vec3 position) : m_aspect_ratio{ aspect_ratio }
		{
		...
		*/
		glm::mat4 PerspectiveCamera2::GetProjMatrix() const
		{
			return glm::perspective(glm::radians(m_fov), m_aspect_ratio, m_near, m_far);
		}

		glm::mat4 PerspectiveCamera2::GetInverseProjMatrix() const
		{
		...
		*/
		glm::mat4 PerspectiveCamera2::GetViewMatrix() const
		{
			return glm::inverse(m_model);
		}

		const glm::mat4& PerspectiveCamera2::GetModelMatrix() const
		{
			return m_model;
		}

		//glm uses column major format => access is m[col][row] = or m[col] to get whole col
		void PerspectiveCamera2::SetPosition(glm::vec3& position)
		{
		...
		glm::vec3 PerspectiveCamera2::GetPosition() const
		{
			//4th col (world pos)
